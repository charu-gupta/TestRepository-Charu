...
def ftnGetQualificationResults(xlsFile, dbCon):
    ...
    try:
        ...
        for rowNo, rowOfCellObjects in enumerate(mysheet['A8':'D12']):
            RFPName =""
            defaultMarks = -1
            ...
            for cellObj in rowOfCellObjects:
                ...
                if (cellObj.column == 'B'):
                    ...
                    if None == cellObj.value or 'Select' == cellObj.value:
                        ...
                        if sql =="":
                            sql='select q.ID_CONSULTANT, ID_QUALIFICATION , name , DOB , email , state , district , address , pan , mobile , alter_mobile , landline from sa_qualification q, sa_consultant c where c.ID_CONSULTANT=q.ID_CONSULTANT and '
                        else:
                            sql = sql + " and "
                        qualiLevel = cellObj.value.strip()
                        sql += "  qualificationLevel in  (select qualificationLevel from master_qualificationlevel where code in  "
                    ...
                    print(cellObj.value, " cellObj.value", qualiLevel, "**************qualiLevel", "sql=== ", sql)
                ...
                if (cellObj.column == 'C'):
                    ...
                    if None == cellObj.value or 'Select' == cellObj.value:
                        ...
                        if sql =="":
                            ...
                            sql = sql + " and "
                        sql += "  qualificationName in  (select qualificationName from master_qualificationname where code in  "

                        value = cellObj.value.strip()
                        arrQualiName = value.split("/")
                        print(arrQualiName, "arrQualiName")
